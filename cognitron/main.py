...
async def request_documents(stc_geck, topic: str):
    SHOULD = 0
    MUST = 1
    MUST_NOT = 2

    search_results = await stc_geck.search([{
    ...
    if documents:
        for document in documents:
            ...
            if 'content' in document_meta:
                ...
                downloaded_dois.append(document_meta['doi'])
                pdf_file = await stc_geck.download(document_meta['cid'])
