...
export class UserDb extends Dexie {
  ...
  async add_search_metrics(search_metrics: SearchMetric) {
    return this.transaction("rw", this.search_metrics, async () => {
      await this.search_metrics.offset(100).delete();
      return this.search_metrics.put(search_metrics);
    });
  }
  ...
  async add_bookmark(bookmark: IBookmark) {
    return this.transaction("rw", this.bookmarks, () => {
      return this.bookmarks.put(bookmark);
    });
  }

  async get_all_bookmarks() {
    return this.transaction("rw", this.bookmarks, () => {
      return this.bookmarks.orderBy("created_at").reverse().toArray();
    });
  }

  async has_bookmark(index_name: string, query: string) {
    return this.transaction("rw", this.bookmarks, async () => {
  ...
  async delete_bookmark(index_name: string, query: string) {
    return this.transaction("rw", this.bookmarks, () => {
      return this.bookmarks.delete([index_name, query]);
    });
  }
}
