...
import { format_bytes, generate_filename, generate_cid_external_links, is_int, cid_local_link } from "@/utils";
  ...
  computed: {
    ...
    extras() {
      ...
      if (this.document.doi) {
        parts.push(
          `<a class="text-decoration-none link-secondary" href="${this.doi_link}">doi:${this.document.doi}</a>`
        );
      }
      if (this.document.referenced_by_count) {
        parts.push(
          `<a class="text-decoration-none link-secondary" href="/#/?q=references.doi:${this.document.doi}&ds=true">🔗 ${this.document.referenced_by_count}</a>`
        );
      }
    ...
    coordinates() {
      ...
      if (container_title) {
        ...
        if (publisher) {
          parts.push("by")
          parts.push(`<a class="text-decoration-none fst-italic link-secondary" href='/#/?q=metadata.publisher:"${encodeURIComponent(publisher)}"'>${publisher}</a>`)
        }
        if (series) {
          parts.push("of")
          parts.push(`<a class="text-decoration-none fst-italic link-secondary" href='/#/?q=metadata.series:"${encodeURIComponent(series)}"&ds=true'>${series}</a>`)
        }
        parts.push("in")
        if (issns) {
          parts.push(`<a class="text-decoration-none fst-italic link-secondary" href='/#/?q=metadata.issns:"${encodeURIComponent(issns[0])}"&ds=true'>${container_title}</a>`)
        } else {
          parts.push(`<a class="text-decoration-none fst-italic link-secondary" href='/#/?q=metadata.container_title:"${encodeURIComponent(container_title)}"&ds=true'>${container_title}</a>`)
        }
      }
    ...
    volume_or_issue() {
      ...
      let with_link = issns && issns.length > 0;
      let filters = [];
      let text = "";
      if (volume && issue) {
      ...
      if (with_link) {
        ...
        return `<a class="text-decoration-none fst-italic link-secondary" href='/#/?q=${filters.join("+")}&ds=true'>${text}</a>`
      } else {
        return text;
      }
    },
