...
export class SearchService {
  ...
  async load_from_store () {
    ...
    const loading_futures = index_configs.map(async (index_config) => {
      await (async () => {
  ...
  async add_index (startup_config: {
    ...
    const index_config = new IndexConfig(
      ...
      Number(index_attributes.created_at),
      startup_config.seed,
  ...
  async search (index_queries: IndexQuery[]) {
    return await this.remote_index_registry.search(index_queries)
  }
  ...
  async install_defaults () {
    ...
    await Promise.all(
      startup_configs.map(async (startup_config) => {
        await (async () => {
  ...
  async get_enabled_index_configs (index_name?: string) {
    ...
    return await meta_db.index_configs
      .filter(
        (index_config) =>
  ...
  async custom_search (
    ...
    if (update_search_metrics) {
      void user_db.add_search_metrics(new SearchMetric(spent))
    }
    return results
  }
